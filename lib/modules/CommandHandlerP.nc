...
implementation{
    task void processCommand(){
        if(! call Queue.empty()){
            ...
            commandID = msg->id;
            //dbg(COMMAND_CHANNEL, "msg->id: \"%d\" before switch\n", msg->id);
            ...
            case CMD_TEST_SERVER:
                dbg(COMMAND_CHANNEL, "Command Type: Client(SERVER)\n"); // (SERVER) added to differentiate debugging
                signal CommandHandler.setTestServer(buff[0]);
                break;

            case CMD_FLOOD:  // added for Project 1 (Flooding)
                dbg(COMMAND_CHANNEL, "Command Type: Flood\n");
                signal CommandHandler.flood(buff[0], buff[1], buff[2], &buff[3]);
                break;
            
            case CMD_ROUTE:  // added for Project 2 (Routing)
                dbg(COMMAND_CHANNEL, "Command Type: Route\n");
                signal CommandHandler.route(buff[0], buff[1], buff[2], &buff[3]);
                break;
